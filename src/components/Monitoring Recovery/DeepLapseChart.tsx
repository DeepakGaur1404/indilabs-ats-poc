...
console.log(dataRecovery,"dataRecovery");
  ...
  console.log(DLData["PREM OS"],"DLData[PREM OS]...........");
  
  useEffect(() => {
   if (activeTabs==="Deep Lapse"&& newselectedCategory==="All"&& DLData.all){
    toFilterData(DLData.all)
   }
   else if (activeTabs==="Deep Lapse"&& newselectedCategory==="Prem_OS"&& DLData["PREM OS"]){
      let ld: StaticDataItem[] = []; 
        if (PremOSCategorie === "<20K" && DLData["PREM OS"]) {
          ld = DLData["PREM OS"].filter((item:any) => item.sub_segment === "<=20k");
        } else if (PremOSCategorie === "20K-50K" && DLData["PREM OS"]) {
          ld = DLData["PREM OS"].filter((item:any) => item.sub_segment === ">20k, <=50k");
        } else if (PremOSCategorie === "50K-150K" && DLData["PREM OS"]) {
          ld = DLData["PREM OS"].filter((item:any) => item.sub_segment === ">50k, <=150k");
        }else if (PremOSCategorie === ">150K" && DLData["PREM OS"]) {
          ld = DLData["PREM OS"].filter((item:any) => item.sub_segment === ">150k");
        }
        
        toFilterData(ld);
   }
 
   else if (activeTabs==="Deep Lapse"&& newselectedCategory==="Prem_Frequency"&& DLData["PREM FREQ"]){
    let ld: StaticDataItem[] = []; 
      if (PremFrequency === "Monthly") {
        ld = DLData["PREM FREQ"].filter((item:any) => item.sub_segment === "Monthly");
      } else if (PremFrequency === "Quarterly") {
        ld = DLData["PREM FREQ"].filter((item:any) => item.sub_segment === "Quarterly");
      } else if (PremFrequency === "Semi Ann") {
        ld = DLData["PREM FREQ"].filter((item:any) => item.sub_segment === "Semi Ann");
      }else if (PremFrequency === "Annual") {
        ld = DLData["PREM FREQ"].filter((item:any) => item.sub_segment === "Annual");
      }
      
      toFilterData(ld);
 }  

 else if (activeTabs==="Deep Lapse"&& newselectedCategory === "Policy_Year"&& DLData["POLICY YEAR"]){
  let ld: StaticDataItem[] = []; 
    if (PolicyYear  === "13M") {
      ld = DLData["POLICY YEAR"].filter((item:any) => item.sub_segment === "13M");
    } else if (PolicyYear  === "25M") {
      ld = DLData["POLICY YEAR"].filter((item:any) => item.sub_segment === "25M");
    } else if (PolicyYear  === "61M") {
      ld = DLData["POLICY YEAR"].filter((item:any) => item.sub_segment === "61M");
    }else if (PolicyYear  === "49M") {
      ld = DLData["POLICY YEAR"].filter((item:any) => item.sub_segment === "49M");
    }
...
else if (activeTabs==="Deep Lapse"&& newselectedCategory==="Product"&& DLData["PRODUCT TYPE"]){
  let ld: StaticDataItem[] = []; 
    if (Product === "Traditional") {
      ld = DLData["PRODUCT TYPE"].filter((item:any) => item.sub_segment === "TRADITIONAL");
    } else if (Product === "ULIP") {
      ld = DLData["PRODUCT TYPE"].filter((item:any) => item.sub_segment === "ULIP");
    } 
