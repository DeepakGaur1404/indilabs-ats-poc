...
const PerformanceRecovery: React.FC = () => {
  ...
  const [staticDataUniquePerformance, setStaticDataUniquePerformance]=useState<any>()
  const [error, setError] = useState<string | null>(null);
  
  useEffect(() => {
    fetchData("performance.dlr_recovery");
     fetchUniqueData("performance.unique_payer")
  }, []);

  const fetchData = async (blob: any) => {
    const url = `https://indilab-apim.azure-api.net/api/performance?blob=performance.dlr_recovery`;
    const headers = {
    ...
    try {
      ...
      console.log(staticDataRecoveryPerformance, "staticDataRecoveryPerformance")
    } catch (err: any) {
      setError(err.message);
    }
  };
  const fetchUniqueData = async (blob: any) => {
    const url = `https://indilab-apim.azure-api.net/api/performance?blob=performance.unique_payer`;
    const headers = {
      "Ocp-Apim-Subscription-Key": "9a4cebcda5b449bdb29fe6b2b75a4dfa",
    };
    ...
    try {
      ...
      const result = await response.json();
      console.log(result.data,"fetched data");
      setStaticDataUniquePerformance(result.data);
